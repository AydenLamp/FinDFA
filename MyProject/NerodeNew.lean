...
/-! ### Left Quotient Charactarization of Nerode -/

theorem nerode_iff_leftQuotient (s₁ s₂ : σ) :
    M.nerode s₁ s₂ ↔ M.acceptsFrom s₁ = M.acceptsFrom s₂ := by
  simp [nerode, Indist, acceptsFrom]
  exact Iff.symm Language.ext_iff
